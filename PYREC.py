...
class Aggregator(nn.Module):
    ...
    def forward(self, ego_embeddings, A_in):
        ...
        #Information Aggregation by bi-interaction:
        # Equation (8) & (9)
        sum_embeddings = self.activation(self.linear1(ego_embeddings + side_embeddings))
        bi_embeddings = self.activation(self.linear2(ego_embeddings * side_embeddings))
        ## this is the second Attention Mechanism used for Information Aggregation 
        if self.attention:
...
class PYREC(nn.Module):
    ...
    def __init__(self, args,
        ...
        #get the parameter about if use attention
        self.attention = args.attention
        #get the parameter about if use knowledgegraph
        ...
        #############################
        ## in current version, the user/project nodes have been merged into entity nodes.
        #############################
        ...
        #create multiple layers for the KG(GNN) model
        self.aggregator_layers = nn.ModuleList()
        for k in range(self.n_layers):
            self.aggregator_layers.append(Aggregator(self.conv_dim_list[k], self.conv_dim_list[k + 1], self.mess_dropout[k], self.aggregation_type, self.attention))
